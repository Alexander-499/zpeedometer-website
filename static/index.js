...
const scrollToSection = (id) => {
  ...
  if (!target) return;
  window.scrollTo({ top: id === "home" ? 0 : target.offsetTop - 100, behavior: "smooth" });
};

highlightNavListItem = item => {
  navHighlight.style.width = `${item.offsetWidth}px`;
  navHighlight.style.transform = `translateX(${item.offsetLeft}px)`;
};
...
window.addEventListener("resize", () => {
  ...
  && highlightNavListItem(document.querySelector("nav li.active"));
});

window.addEventListener("load", () => {
  const active = document.querySelector("nav li.active");
  if (active) highlightNavListItem(active);
